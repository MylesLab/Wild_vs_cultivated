...
wilcox.test(abc_data)
#Significant results:
#Precocity, flowering date, harvest date, weight, acidity, phenolic content
#Exact P values are printed out in the above loop. 
# Note: these tests do not adjust the p value, so the p values in the publication must be adjusted (divided by the number of observations) post-hoc.

# to retrive exact P-values: 
test_prec = wilcox.test(abc_data$Precocity[index_wild],abc_data$Precocity[index_dess])
test_prec$p.value
test_flow = wilcox.test(abc_data$Flowering.Date[index_wild],abc_data$Flowering.Date[index_dess])
test_flow$p.value
test_harv = wilcox.test(abc_data$Harvest.Date[index_wild],abc_data$Harvest.Date[index_dess])
test_harv$p.value
test_weight = wilcox.test(abc_data$Weight[index_wild],abc_data$Weight[index_dess])
test_weight$p.value
test_acid = wilcox.test(abc_data$Acidity[index_wild],abc_data$Acidity[index_dess])
test_acid$p.value
test_phenol = wilcox.test(abc_data$Phenolic.Content[index_wild],abc_data$Phenolic.Content[index_dess])
test_phenol$p.value
test_xyz = wilcox.test(abc_data$Change.in.firmness.during.storage[index_wild],abc_data$Change.in.firmness.during.storage[index_dess])
test_xyz$p.value
